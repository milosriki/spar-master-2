...
export function EnhancedHabitTracker({ onHabitComplete, onHabitMissed }: EnhancedHabitTrackerProps) {
  ...
  const calculateTaskColor = (taskValue: number): 'red' | 'orange' | 'yellow' | 'green' | 'blue' | 'purple' => {
    if (taskValue < -10) return 'red';
    if (taskValue < -1) return 'orange';
    if (taskValue < 1) return 'yellow';
    if (taskValue < 5) return 'green';
    if (taskValue < 10) return 'blue';
    return 'purple';
  };
    ...
    setHabits(prev => prev.map(habit => {
      ...
      if (habit.type === 'habit') {
        if (action === 'positive') {
          ...
          updatedHabit.taskValue = (habit.taskValue || 0) + 1;
          updatedHabit.taskColor = calculateTaskColor(updatedHabit.taskValue);
          onHabitComplete(habitId, habit.xpValue, habit.goldReward);
        } else if (action === 'negative') {
          updatedHabit.negativeCount = (habit.negativeCount || 0) + 1;
          updatedHabit.taskValue = (habit.taskValue || 0) - 1;
          updatedHabit.taskColor = calculateTaskColor(updatedHabit.taskValue);
          if (habit.hpDamage && onHabitMissed) {
            onHabitMissed(habitId, habit.hpDamage);
          }
        }
        ...
        updatedHabit.taskValue = (habit.taskValue || 0) + 1;
        updatedHabit.taskColor = calculateTaskColor(updatedHabit.taskValue);
        updatedHabit.isCompleteToday = true;
        onHabitComplete(habitId, habit.xpValue, habit.goldReward);
  ...
  const renderHabitCard = (habit: Habit) => (
    <Card key={habit.id} className={cn('mb-3', getTaskColorClass(habit.taskColor))}>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {habit.type === 'daily' && habit.isCompleteToday && (
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Complete habits to earn XP and gold. Miss dailies to lose HP!
            </CardDescription>
          </div>
